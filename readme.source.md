...
<div style={{ width: '100%', height: '100%', background: '#08080c', display: 'flex', alignItems: 'center', fontFamily: 'Inter, sans-serif', position: 'relative', overflow: 'hidden', borderRadius: 16, border: '1px solid rgba(220,30,30,0.18)' }}>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', marginLeft: 168, gap: 8, zIndex: 10 }}>
    ...
    <div style={{ fontSize: 15, color: 'rgba(255,180,180,0.8)', fontWeight: 400, letterSpacing: '0.3px' }}>Full Stack Engineer · Gen AI · Machine Learning</div>
...
<div style={{ display: 'flex', flexDirection: 'row', gap: 12, width: '100%', height: '100%', fontFamily: 'Inter, sans-serif' }}>
  ...
  {[
    ...
    { label: 'STARS', value: (github && github.totalStars) || '4', color: '#f87171' },
    { label: 'COMMITS', value: (github && github.totalCommits) || '100+', color: '#fca5a5' },
  ].map((stat, i) => (
    <div key={i} className="stat-card" style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#08080c', borderRadius: 14, border: '1px solid rgba(220,30,30,0.2)' }}>
      <span style={{ fontSize: 28, fontWeight: 700, color: stat.color || '#e2e8f0' }}>{stat.value}</span>
      <span style={{ fontSize: 10, color: 'rgba(255,180,180,0.45)', letterSpacing: 3, marginTop: 4 }}>{stat.label}</span>
    </div>
  ))}
</div>
```
...
<div style={{ position: 'relative', width: '100%', height: '100%', background: '#08080c', borderRadius: 14, overflow: 'hidden', fontFamily: 'Inter, sans-serif', border: '1px solid rgba(220,30,30,0.2)', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 28px', boxSizing: 'border-box' }}>
  <style>{`
    @keyframes tech-glow { 0%, 100% { transform: translateX(0px); opacity: 0.6; } 50% { transform: translateX(200px); opacity: 0.9; } }
    #tech-orb1 { animation: tech-glow 10s ease-in-out infinite; }
    #tech-orb2 { animation: tech-glow 13s ease-in-out infinite reverse; }
  `}</style>
  <svg width="860" height="140" style={{ position: 'absolute', top: 0, left: 0 }}>
    <defs>
      <radialGradient id="tg1" cx="50%" cy="50%" r="50%"><stop offset="0%" stopColor="rgba(200,20,20,0.3)" /><stop offset="70%" stopColor="rgba(200,20,20,0)" /></radialGradient>
      <radialGradient id="tg2" cx="50%" cy="50%" r="50%"><stop offset="0%" stopColor="rgba(180,0,0,0.25)" /><stop offset="70%" stopColor="rgba(180,0,0,0)" /></radialGradient>
    </defs>
    <ellipse id="tech-orb1" cx="100" cy="70" rx="180" ry="100" fill="url(#tg1)" />
    <ellipse id="tech-orb2" cx="700" cy="70" rx="180" ry="100" fill="url(#tg2)" />
  </svg>
  <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 28, zIndex: 10 }}>
    ...
    <div style={{ width: 1, height: 55, background: 'rgba(220,30,30,0.25)' }} />
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
...
<SocialMediaButton
  ...
  gradientStops={[
    ...
    { offset: '50%', color: '#880000' },
    { offset: '100%', color: '#cc2222' },
  ]}
/>
```
